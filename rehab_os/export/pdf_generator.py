...
def generate_note_pdf(
    *,
    ...
    pdf.cell(0, 4, f"Generated by RehabOS on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC", new_x="LMARGIN", new_y="NEXT")
...
def _render_table(pdf: FPDF, headers: list[str], rows: list[list[str]]) -> None:
    ...
    for row in rows:
        for cell in row:
            sanitized = _sanitize(str(cell))
            pdf.cell(col_w, 5, (sanitized[:37] + "...") if len(sanitized) > 40 else sanitized, border=1)
        pdf.ln()
    pdf.ln(2)
